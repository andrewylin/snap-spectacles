...
export class MapComponent extends BaseScriptComponent {
  ...
  onStart() {
    if (!this.componentPrefab) {
      print("MapComponent: ERROR - componentPrefab is null! Check Inspector assignment.");
      return;
    }
    ...
    if (!this.mapController) {
      print("MapComponent: ERROR - MapController not found! Check MapController prefab.");
      return;
    }
    ...
    try {
      // Initialize only GPS and speed tracking, skip map rendering
      this.mapController.initializeMinimal(mapParameters);
    } catch (error) {
      print(`MapComponent: ERROR during initialization: ${error}`);
    }
  }
...
/*   subscribeOnMapPinRemoved(fn: callback<MapPin>): void {
  ...
  /**
   * Return the user's current speed in miles per hour
   */
  getUserSpeed(): number {
    return this.mapController.getUserSpeed();
  }
  ...
  setNetworkFeaturesEnabled(enabled: boolean): void {
    this.mapController.setNetworkFeaturesEnabled(enabled);
  }
