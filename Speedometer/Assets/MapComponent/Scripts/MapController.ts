...
export class MapController extends BaseScriptComponent {
  ...
  private shouldFollowMapLocation = true; // Default to following user
  ...
  // Speed calculation
  private previousLocation: GeoPosition;
  private previousLocationTime: number = 0;
  ...
  initialize(mapParameters: MapParameter, startedAsMiniMap: boolean): void {
    ...
    this.fetchLocation((location: GeoPosition) => {
      if (!this.mapParameters.setMapToCustomLocation) {
        ...
        this.userLocation = location; // Set initial user location
      }
      
      // Initialize speed tracking
      this.previousLocation = this.copyGeoPosition(location);
      this.previousLocationTime = getTime();
      this.currentSpeed = 0;
  ...
  /**
   * Minimal initialization - only GPS and speed tracking, no map rendering
   */
  initializeMinimal(mapParameters: MapParameter): void {
    log.i("Initializing Map Controller (Minimal - GPS + Speed Only)");
    this.locationService = GeoLocation.createLocationService();
    this.locationService.onNorthAlignedOrientationUpdate.add(
      this.handleNorthAlignedOrientationUpdate.bind(this)
    );
    ...
    // Create minimal objects needed for location updates
    this.createMinimalObjects();

    // Skip map rendering - only set up GPS and speed tracking
    this.fetchLocation((location: GeoPosition) => {
      if (!this.mapParameters.setMapToCustomLocation) {
        this.mapLocation = location;
        this.userLocation = location; // Set initial user location
      ...
      // Create minimal map grid for pinOffsetter (needed for location updates)
      this.createMinimalMapGrid();

      // Set up update loop for GPS and speed tracking
      this.updateDispatcher
        .createUpdateEvent("UpdateEvent")
        .bind(this.onUpdate.bind(this));

      log.i("Map Controller initialized (Minimal - GPS + Speed Only)");
      this.isInitialized = true;
    });
  }
  ...
  private fetchLocation(callback: callback<GeoPosition>) {
    // Commented out to reduce log spam - uncomment for GPS debugging
    // print("MapController: fetchLocation called, requesting GPS position...");
    this.locationService.getCurrentPosition(
      (geoPosition) => {
        // Commented out to reduce log spam - uncomment for GPS debugging
        // print(`MapController: GPS position received successfully: lat=${geoPosition.latitude.toFixed(6)}, lon=${geoPosition.longitude.toFixed(6)}`);
        callback(geoPosition);
      },
      (error) => {
        print(`MapController: ERROR fetching GPS location: ${error}`);
  ...
  /**
   * Get the current user speed in miles per hour
   */
  getUserSpeed(): number {
   ...
   */
  private calculateSpeed(currentLocation: GeoPosition, previousLocation: GeoPosition, timeDelta: number): number {
    ...
    const timeInHours = timeDelta / 3600;
  ...
  /**
   * Create a copy of a GeoPosition object
   */
  private copyGeoPosition(geoPosition: GeoPosition): GeoPosition {
    const copy = GeoPosition.create();
    copy.latitude = geoPosition.latitude;
    copy.longitude = geoPosition.longitude;
    copy.altitude = geoPosition.altitude;
    copy.timestamp = geoPosition.timestamp;
    return copy;
  }
   ...
   * Create minimal objects needed for location updates
   */
  private createMinimalObjects() {
    // Create minimal objects that are referenced in location updates
    this.mapPinsAnchor = global.scene.createSceneObject("MinimalPinsAnchor");
    this.mapPinsAnchor.getTransform().setLocalPosition(vec3.zero());
    
    // Create a minimal screen transform for the pins anchor
    const screenTransform = this.mapPinsAnchor.createComponent("ScreenTransform");
    screenTransform.anchors.setCenter(new vec2(0.5, 0.5));
  }

  /**
   * Minimal map grid setup - only creates pinOffsetter for location updates
   */
  private createMinimalMapGrid() {
    // Create minimal config needed for pinOffsetter
    this.config = MapConfig.makeConfig(
      this.mapPinsAnchor, // Use the minimal pins anchor
      this.mapPinsAnchor.getComponent("ScreenTransform"), // Use the pins anchor's screen transform
      this.mapPinsAnchor.getComponent("ScreenTransform"), // Use the same for grid transform
      this.mapTilePrefab, // Use the existing tile prefab (required)
      this,
      false, // No scrolling
      0, // No friction
      1 // Minimum tile count
    );

    // Set up minimal zoom for pinOffsetter
    this.setUpMinimalZoom();
  }
  ...
  /**
   * Minimal zoom setup - only creates pinOffsetter for location updates
   */
  private setUpMinimalZoom() {
    this.referencePositionLocationAsset = LocationAsset.getGeoAnchoredPosition(
      this.mapLocation.longitude,
      this.mapLocation.latitude
    ).location.adjacentTile(0, 0, this.mapParameters.zoomOffet);
    ...
    // Skip grid view updates since we don't have a grid
    // this.gridView.handleUpdateConfig(this.config);
  }
  ...
  spawnUserPin(
    ...
    // Only proceed if userPin was created successfully
    if (this.userPin && this.userPin.screenTransform) {
      ...
      // Only do gridView operations if we have a grid view
      if (this.gridView && this.pinOffsetter) {
        this.pinOffsetter.layoutScreenTransforms(this.gridView);
      }
    }
  }
  ...
  private setNewUserPosition(location: GeoPosition): void {
    ...
    if (this.previousLocation && this.previousLocationTime > 0) {
      const timeDelta = currentTime - this.previousLocationTime;
      
              // Only calculate speed if enough time has passed (avoid division by very small numbers)
        if (timeDelta >= 0.2) { // At least 0.2 seconds between updates for more responsive speed tracking
          this.currentSpeed = this.calculateSpeed(location, this.previousLocation, timeDelta);
          
          // Update previous location for next calculation
          this.previousLocation = this.copyGeoPosition(location);
          this.previousLocationTime = currentTime;
        }
      } else {
        // First location - no speed calculation possible yet
        this.previousLocation = this.copyGeoPosition(location);
        this.previousLocationTime = currentTime;
        this.currentSpeed = 0;
      }
    ...
    // Only do pinOffsetter operations if we have a user pin and full map system
    if (this.pinOffsetter && this.userPin && this.userPin.screenTransform) {
      this.pinOffsetter.bindScreenTransformToLocation(
        this.userPin.screenTransform,
        location.longitude,
        location.latitude
  ...
  private setNewMapLocation(location: GeoPosition): void {
    ...
    // Only do pinOffsetter operations if we have a full map system
    if (this.pinOffsetter && this.mapPinsAnchor) {
      this.pinOffsetter.bindScreenTransformToLocation(
    ...
    if (this.gridView) {
      ...
      // Always follow the user location by default unless explicitly disabled
  ...
  showNearbyPlaces(category: string[]): void {
    if (!this.enableNetworkFeatures || !this.placesProvider) {
      log.i("Network features disabled or places provider not available");
