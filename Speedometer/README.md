...
- **Revamped Map Component** : Users can create and manipulate map pins to any desired location. This is achieved through a revamped Map Component, providing seamless interaction and pin management. The new component aims to minimize effort to set up and removes the need to use additional orthographic camera to render the map texture which optimizes performance on the Spectacles device. Additionally it provides the following features:
  ...
  - **User Following** : The map automatically follows the user's current location, keeping them centered on the map for optimal navigation.
  - **Speed Display** : Real-time speed tracking showing the user's current speed in miles per hour, with automatic visibility control based on movement.
...
The MapComponent now includes built-in speed tracking and display functionality:

```typescript
@component
export class SpeedExample extends BaseScriptComponent {
  @input
  private mapComponent: MapComponent;
  
  onStart() {
    // Get current user speed in mph
    const currentSpeed = this.mapComponent.getUserSpeed();
    print(`Current speed: ${currentSpeed} mph`);
    
    // Subscribe to user location updates
    this.mapComponent.subscribeOnUserLocationFirstSet((location) => {
...
For complete speed display setup, see the [Speed Display Documentation](./Assets/MapComponent/README_SpeedDisplay.md).
    });
  }
}
```
